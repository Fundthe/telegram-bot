...
async def get_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.message.text.strip() in ["🔙 Назад", "🏠 В начало"]:
            return await start(update, context)
        try:
            rate = float(update.message.text.strip().replace('%', ''))
            user_data_temp[update.effective_chat.id] = {'rate': rate}
            await update.message.reply_text(
                "⏱ Теперь напиши, раз в сколько часов начисляется процент (например: 4)",
                reply_markup=ReplyKeyboardMarkup([["🏠 В начало"]], resize_keyboard=True)
            )
            return HOURS
        except ValueError:
            await update.message.reply_text(
                "❌ Пожалуйста, введи число. Например: 0.5",
                reply_markup=ReplyKeyboardMarkup([["🏠 В начало"]], resize_keyboard=True)
            )
            return RATE
